...
- **PID Controller**:
  ...
  - To upload the PID tuning code, navigate to the PID tuning directory and run the following:
    ...
    pio run -e esp32 -t upload
    ```
  - A custom [config.h](https://github.com/Labeeb1234/amr/blob/main/AMR_Hardware/config/custom/esp32_config.h) file is created in order to consolidate all the PINOUT parameters of all the connections to the esp32 board. The default configs we used for our system is shown below:
  ```cpp
  ...
  /*
    ...
    */
    ...
    #define CONTROL_TIMER 20 # in [ms] lower layer update_rate 50Hz
    #define BOARD_INIT {\ 
