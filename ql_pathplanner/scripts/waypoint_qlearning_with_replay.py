...
class QLearningNode(Node):
    def __init__(self):
        ...
        self.start_q_table = self.load_qtable(f"q_table_230") # best for straight line
        self.q_table = self.start_q_table
        self.verify_qtable()

        # goal params
        self.goal_position = [5.5, 0.0, 0.0]
    ...
    def randomize_goal(self):
        ...
        self.goal_position[0] = 0.0  # Random int x value in range [1, 3]
        self.goal_position[1] = float(np.random.randint(1, 4))  # Random int y value in range [1, 3]
    ...
    def save_qtable(self, episode, q_table):
        os.makedirs(name="new_dir/qtables", exist_ok=True)
        filename = f"new_dir/qtables/q_table_{episode}.npy"
    ...
    def inference_loop(self):
        ...
        if self.distance_to_goal < self.goal_tolerance and self.angle_diff < self.orientation_tolerance:
